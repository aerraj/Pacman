...
const ghosts=[new Ghost({
    ...
    }), new Ghost({position: {x:Boundary.width*8+Boundary.width/2,y:Boundary.height+Boundary.height/2},
        velocity: {x:5,y:Ghost.speed}, color: "pink"}),new Ghost({position: {x:Boundary.width+Boundary.width/2,y:Boundary.height*5+Boundary.height/2},
        velocity: {x:5,y:Ghost.speed}, color: "green" })
]
...
map.forEach((row,i)=>{ 
    ...
    row.forEach((symbol,j)=>{
        ...
        case 'p':
            powerUps.push(
                new PowerUp({
                    position: {x: j * Boundary.width + Boundary.width / 2,
                               y: i * Boundary.height + Boundary.height / 2}
                })
            )
...
//colliding player with other ghosts
  ...
  if(Math.hypot(ghost.position.x- player.position.x,ghost.position.y- player.position.y)<(ghost.radius+player.radius))
     {
       ...
       alert(" You Lost click anywhere to replay")
...
{   cancelAnimationFrame(animationId)
    ...
    alert("You win click anywhere to replay")
...
//to detect collisions between player and pellets
...
{
    boundary.draw();
    if(circleCollidesWithRect({circle: player, rectangle:boundary}))
    {
